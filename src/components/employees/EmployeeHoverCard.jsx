      ...
      <HoverCardContent className="w-96">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex items-center">
            ...
            <span className="text-muted-foreground">
              {  t('joined')}: {formatDate(employee.joined_at)}
            </span>
          </div>
          <div className="flex items-center">
            <Briefcase className="mr-2 h-4 w-4 opacity-70" />
            <span className="text-muted-foreground">
              {  t('experience')}: {calculateExperience(employee.joined_at)}
            </span>
          </div>
          <div className="flex items-center">
            <LogIn className="mr-2 h-4 w-4 opacity-70" />
            <span className="text-muted-foreground">
              {  t('last-signin')}: {formatDateTime(employee.last_login)}
            </span>
          </div>
          <div className="flex items-center">
            <PenLine className="mr-2 h-4 w-4 opacity-70" />
            <span className="text-muted-foreground">
              {  t('last-updated')}: {formatDateTime(employee.updatedAt)}
            </span>
          </div>
        </div>
      </HoverCardContent>
